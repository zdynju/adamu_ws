...
async def run_edge_top_grasp_demo(
    ...
    # 先沿 +Y（右臂视角）贴近棱边
    if not await arm_ctrl.execute_pilz_straight_line('right', (0.0, 0.03, 0.0), vel=0.05):
        arm_ctrl.get_logger().error('贴近阶段失败')
        return

    arm_ctrl.get_logger().info('=== [Demo] Step 5: 直线下探至上棱位置 ===')
    if not await arm_ctrl.execute_pilz_straight_line('right', (0.0, 0.0, -0.02), vel=0.03):
    ...
    if not await arm_ctrl.execute_pilz_straight_line('right', (0.0, 0.0, 0.05), vel=0.04):
